		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					//direction: "rtl",
					url:'../../EvalSystState_manageEvalSystState.action?oper=search',
					//data: grid_data,
					datatype: "json",
					mtype:"post",
					height: 445,
					colNames:['id','体系状态编码','体系状态描述'],
					colModel:[
						{name:'id',index:'id', width:60, sorttype:"int",key:true, editable: false,search:false,hidden:true},
						{name:'evalSystStateNum',index:'evalSystStateNum', width:60, sorttype:"int",editable: true,
						editrules:{required:true,custom:true,custom_func:evalSystStateNumCheck},formoptions:{elmsuffix:"<font color='red'>*</font>"}},
						
						{name:'evalSystStateDes',index:'evalSystStateDes',width:90, editable: true,editoptions:{size:"20",maxlength:"5"},editrules:{required:true},formoptions:{elmsuffix:"<font color='red'>*</font>"}}
					...
					editurl: "../../EvalSystState_manageEvalSystState.action",//nothing is saved
					caption: "评价体系状态",
				...
				//验证评价体系编号的唯一性
				function evalSystStateNumCheck(value,colname){
					var evalSystStateNumint=parseInt(value);
					//alert(EvalSystStateint);
					var isnum=0;
					//alert(parseInt(value)+parseInt("1"));
					for(var i=0;i<10;i++){
						if(i==evalSystStateNumint){
							isnum=1;
						}
					}
					if(isnum==0){
						return [false,"体系状态编码必须为0-9之间的数字!"];
					}else{
					...
					$.ajax({
						type : "get",
						async: false,
						url : "../../EvalSystState_checkEvalSystStateNum.action?evalSystStateNum="+value+"&id="+id,
						dataType : "json",
				...
				jQuery(grid_selector).jqGrid('navGrid',grid_selector,
					...
					},
					{
						...
						url: '../../EvalSystState_manageEvalSystState.action', 
						mtype: 'POST',
						closeAfterEdit: true,
