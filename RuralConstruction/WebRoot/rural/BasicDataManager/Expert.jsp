...
<![endif]-->
		...
		<script src="assets/js/fuelux/fuelux.tree.min.js"></script>
			...
			jQuery(function($){
				...
				$.ajax({
						url: "../../exp_singTree.action",//tree_singTree.action
						async:false,  
						type: 'POST',
						dataType: 'json',
				...
				jQuery(grid_selector).jqGrid({
					...
					colModel:[
						...
						{name:'ExpertWorkTel',index:'ExpertWorkTel', width:55,editable: true,editoptions:{size:"20",maxlength:"13"},formoptions:{elmsuffix:"<font color='red'>*</font>"},editrules:{required:true,custom:true,custom_func:WorkTelCheck}},
						{name:'ExpertMobTel',index:'ExpertMobTel', width:55,editable: true,editoptions:{size:"20",maxlength:"11"},formoptions:{elmsuffix:"<font color='red'>*</font>"},editrules:{required:true,custom:true,custom_func:MobTelCheck}},
				...
				function WorkTelCheck(value,colname){
                   ...
                   // var ss = s.replace(reg, ""); 
					...
					if(value.length!=0){
							...
							reg=/^[0]{1}[6]{1}[3]{1}[4]{1}[-][0-9]{8}$/;
						
					}
						
					if(!reg.test(value)){
						return [false,"固定电话必须由'0634-'组成,并且一共12位!"];
					}else{
					return [true,""];
				}
