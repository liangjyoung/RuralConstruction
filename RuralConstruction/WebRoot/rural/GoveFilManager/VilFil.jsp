...
<![endif]-->
		...
		<script type="text/javascript"><!--
			...
			jQuery(function($){
				...
				$.ajax({
						url: "../../vilfil_GoveFilClfy.action",//tree_singTree.action
						async:false,  
						type: 'POST',
						dataType: 'json',
			...
			var treeDataSource = new DataSourceTree({data: tree_data});
					...
					jQuery(grid_selector).jqGrid("setGridParam", {
		                	...
		                	url :'../../vilfil_manageVilFil.action?oper=search&JQID='+ids,
		                	editurl:'../../vilfil_manageVilFil.action?JQID='+ids,
			...
			$.widget("ui.dialog", $.extend({}, $.ui.dialog.prototype, {
						...
						buttons: [ 
							
							{
								...
								click: function() {
									...
									$.ajax({
										url: "../../vilfil_addMenusAsTree.action",//tree_singTree.action
										async:false,  
										type: 'POST',
										dataType: 'json',
										data:{ 
						...
						buttons: [
							{
								...
								click: function() {
								 ...
								 for (var i in items) {
									...
									$.ajax({
										url: "../../vilfil_delMenusAsTree.action",//tree_singTree.action
										async:false,  
										type: 'POST',
										dataType: 'json',
										data:{ 
				...
				jQuery(grid_selector).jqGrid({
					...
					datatype: "json",
					height: 360,
					colNames:[ '村庄文件ID','所属村','村庄文件编号','村庄文件名称','村庄文件标识','村庄文件颁发时间','村庄文件颁发单位','村庄文件说明'],
					colModel:[
						{name:'VilFilID',index:'VilFilID',width:40, sorttype:"int", editable: false, key:true,hidden:true},
						...
						{name:'VilBasicInfoID',index:'VilBasicInfoID',width:20, sortable:false,editable: true,edittype:"select",editoptions:{dataUrl:"../../selbeloDivi_findAll1.action" }},
						{name:'VilFilNum',index:'VilFilNum',editoptions:{size:"20",maxlength:"12"}, width:35,editable: true,formoptions:{elmsuffix:"<font color='red'>*</font>"},editrules:{required:true,custom:true,custom_func:VilFilNumCheck}},
						{name:'VilFilName',index:'VilFilName',editoptions:{size:"20",maxlength:"30"}, width:35, editable: true},
						{name:'VilFilIdef',index:'VilFilIdef', editoptions:{size:"20",maxlength:"50"},width:40, sortable:false,editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}},
						{name:'VilFilMadTim',index:'VilFilMadTim', width:40,editable: true,sorttype:"date",unformat: pickDate},
						{name:'VilFilPubComp',index:'VilFilPubComp',editoptions:{size:"20",maxlength:"50"}, width:40,editable: true,editoptions:{size:"20",maxlength:"30"}},
						{name:'VilFilExpl',index:'VilFilExpl',editoptions:{size:"20",maxlength:"200"},width:40,editable: true,editoptions:{size:"20",maxlength:"30"}},
				    ...
				    prmNames : {
					    ...
					    id:"VilFilID", // 表示当在编辑数据模块中发送数据时，使用的id的名称  
				...
				function VilFilNumCheck(value,colname){
				
						var VilFilID = $(grid_selector).jqGrid("getRowData",$(grid_selector).jqGrid('getGridParam','selrow')).VilFilID
						var flag;
						$.ajax({
							type : "get",
							async: false,
							url : "../../vilfil_checkVilFilNum.action?VilFilNum="+value+"&VilFilID="+VilFilID,
							dataType : "json",
							success : function(data) {
								if (data.success) {
									flag = true;
				...
				function style_edit_form(form) {
					...
					form.find('input[name=VilFilMadTim]').datepicker({format:'yyyy-mm-dd' , autoclose:true})
