...
<![endif]-->
		...
		<script type="text/javascript">
			jQuery(function($){
				...
				$.ajax({
						url: "../../citysfil_GoveFilClfy.action",//tree_singTree.action
						async:false,  
						type: 'POST',
						dataType: 'json',
			...
			var treeDataSource = new DataSourceTree({data: tree_data});
					...
					jQuery(grid_selector).jqGrid("setGridParam", {
		                	...
		                	url :'../../citysfil_manageCitysFil.action?oper=search&JQID='+ids,
		...
		$.widget("ui.dialog", $.extend({}, $.ui.dialog.prototype, {
						...
						buttons: [ 
							
							{
								...
								click: function() {
									...
									$.ajax({
										url: "../../citysfil_addMenusAsTree.action",//tree_singTree.action
										async:false,  
										type: 'POST',
										dataType: 'json',
										data:{ 
						...
						buttons: [
							{
								...
								click: function() {
								 ...
								 for (var i in items) {
									...
									$.ajax({
										url: "../../citysfil_delMenusAsTree.action",//tree_singTree.action
										async:false,  
										type: 'POST',
										dataType: 'json',
										data:{ 
				...
				jQuery(grid_selector).jqGrid({
					...
					colModel:[
						{name:'CitysFilID',index:'CitysFilID',width:40, sorttype:"int", editable: false, key:true,hidden:true},
						...
						{name:'CityBasicInfoID',index:'CityBasicInfoID',width:20, sortable:false,editable: true,edittype:"select",editoptions:{dataUrl:"../../selbeloDivi_findAll4.action" }},
						{name:'CitysFilNum',index:'CitysFilNum', editoptions:{size:"20",maxlength:"12"},width:35,editable: true,formoptions:{elmsuffix:"<font color='red'>*</font>"},editrules:{required:true,custom:true,custom_func:CitysFilNumCheck}},
						{name:'CitysFilName',index:'CitysFilName', editoptions:{size:"20",maxlength:"30"},width:35, editable: true},
						{name:'CitysFilIdef',index:'CitysFilIdef', editoptions:{size:"20",maxlength:"50"},width:40, sortable:false,editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}},
				    ...
				    prmNames : {
					    ...
					    id:"CitysFilID", // 表示当在编辑数据模块中发送数据时，使用的id的名称  
				...
				function CitysFilNumCheck(value,colname){
					var CitysFilID = $(grid_selector).jqGrid("getRowData",$(grid_selector).jqGrid('getGridParam','selrow')).CitysFilID
					var flag;
					$.ajax({
						type : "get",
						async: false,
						url : "../../citysfil_checkCitysFilNum.action?CitysFilNum="+value+"&CitysFilID="+CitysFilID,
						dataType : "json",
						success : function(data) {
							if (data.success) {
								flag = true;
