...
<![endif]-->
		...
		<script type="text/javascript"><!--
			jQuery(function($){
				...
				$.ajax({
						url: "../../countryfil_GoveFilClfy.action",//tree_singTree.action
						async:false,  
						type: 'POST',
						dataType: 'json',
			...
			var treeDataSource = new DataSourceTree({data: tree_data});
					...
					jQuery(grid_selector).jqGrid("setGridParam", {
		                	...
		                	url :'../../countryfil_manageCountryFil.action?oper=search&JQID='+ids,
		                	editurl:'../../countryfil_manageCountryFil.action?JQID='+ids
						...
						buttons: [ 
							
							{
								...
								click: function() {
									...
									$.ajax({
										url: "../../countryfil_addMenusAsTree.action",//tree_singTree.action
										async:false,  
										type: 'POST',
										dataType: 'json',
										data:{ 
						...
						buttons: [
							{
								...
								click: function() {
								 ...
								 for (var i in items) {
									...
									$.ajax({
										url: "../../countryfil_delMenusAsTree.action",//tree_singTree.action
										async:false,  
										type: 'POST',
										dataType: 'json',
										data:{ 
				...
				jQuery(grid_selector).jqGrid({
					...
					datatype: "json",
					height: 360,
					colNames:[ '县文件ID','所属县','县文件编号','县文件名称','县文件标识','县文件颁发时间','县文件颁发单位','县文件说明'],
					colModel:[
						{name:'CountryFilID',index:'CountryFilID',width:40, sorttype:"int", editable: false, key:true,hidden:true},
						...
						{name:'CountryBasicInfoID',index:'CountryBasicInfoID',width:20, sortable:false,editable: true,edittype:"select",editoptions:{dataUrl:"../../selbeloDivi_findAll3.action" }},
						{name:'CountryFilNum',index:'CountryFilNum',editoptions:{size:"20",maxlength:"12"}, width:35,editable: true,formoptions:{elmsuffix:"<font color='red'>*</font>"},editrules:{required:true,custom:true,custom_func:CountryFilNumCheck}},
						{name:'CountryFilName',index:'CountryFilName',editoptions:{size:"20",maxlength:"30"}, width:35, editable: true},
						{name:'CountryFilIdef',index:'CountryFilIdef',editoptions:{size:"20",maxlength:"50"}, width:40, sortable:false,editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}},
						{name:'CountryFilMadTim',index:'CountryFilMadTim', width:40,editable: true,sorttype:"date",unformat: pickDate},
						{name:'CountryFilPubComp',index:'CountryFilPubComp', editoptions:{size:"20",maxlength:"50"},width:40,editable: true,editoptions:{size:"20",maxlength:"30"}},
						{name:'CountryFilExpl',index:'CountryFilExpl',editoptions:{size:"20",maxlength:"200"},width:40,editable: true,editoptions:{size:"20",maxlength:"30"}},
				    ...
				    prmNames : {
					    ...
					    id:"CountryFilID", // 表示当在编辑数据模块中发送数据时，使用的id的名称  
		...
		function CountryFilNumCheck(value,colname){
					var CountryFilID = $(grid_selector).jqGrid("getRowData",$(grid_selector).jqGrid('getGridParam','selrow')).CountryFilID
					var flag;
					$.ajax({
						type : "get",
						async: false,
						url : "../../countryfil_checkCountryFilNum.action?CountryFilNum="+value+"&CountryFilID="+CountryFilID,
						dataType : "json",
						success : function(data) {
							if (data.success) {
								flag = true;
				...
				jQuery(grid_selector).jqGrid('navGrid',grid_selector,
					...
					},
					{
						...
						afterSubmit: function (xhr, postdata) {
						    var rq = JSON.parse(xhr.responseText);
						 	 bootbox.alert(rq.success);
				...
				function style_edit_form(form) {
					...
					form.find('input[name=CountryFilMadTim]').datepicker({format:'yyyy-mm-dd' , autoclose:true})
