...
<![endif]-->
	 ...
	 function ExpertTask(rowrecid) {
	 //alert(rowrecid);
			var consoleDlg = $("#consoleDlg");
			consoleDlg.html("");  
			var a1 = "<iframe src='DistExpert.jsp?VilEvalScoID="+rowrecid+"' style='width: 100%;height: 100%' scrolling='auto' frameborder='0'></iframe>";
			consoleDlg.append(a1);
			consoleDlg.dialog("option", "title", "分配专家").dialog("open");
				
			};
			
	function ExpertScore(rowID){
	//alert(rowID);
	
		var consoleDlg = $("#consoleDlg");
		consoleDlg.html("");  
		var a1 = "<iframe src='ScoreDetails.jsp?EvalExpertDistID="+rowID+"' style='width: 100%;height: 100%' scrolling='auto' frameborder='0'></iframe>";
		consoleDlg.append(a1);
		consoleDlg.dialog("option", "title", "专家打分明细").dialog("open");
			...
			$("#consoleDlg").dialog({
				...
				$.ajax({
						url: "../../experteval_queryAllCompartmentAsTree.action",//tree_singTree.action
						async:false,  
						type: 'POST',
						dataType: 'json',
				...
				var treeDataSource = new DataSourceTree({data: tree_data});
	 			...
	 			$('#tree1').on('selected', function(evt, data) {
					...
					mtype:"post",
					height: 120,
					colNames:[ '','数据采集时间','调查员姓名','专家打分合计'],
					colModel:[
					    {name:'VilEvalScoID',index:'VilEvalScoID', width:10,  editable: true,key:true,hidden:true},
					    {name:'CollTim',index:'CollTim', width:50, editable: true},  
						{name:'InvgatName',index:'InvgatName', width:60,  editable: true},
						{name:'ExpertEvalTotGra',index:'ExpertEvalTotGra', width:140, sortable:false,editable: true} 
					], 
					...
					onSelectRow: function (){   
 					...
 					jQuery(grid_selector1).jqGrid("setGridParam", {
		                url :'../../expertdis_manageExpertdis.action?oper=search&JQID='+rowID,
		   				//editurl:'../../expertdis_manageExpertdis.action?JQID='+rowID	
 		             });
 		            jQuery(grid_selector1).jqGrid("setCaption","专家分配情况").trigger("reloadGrid");
					//alert(rowID);
				...
				jQuery(grid_selector).jqGrid('navGrid',pager_selector,
					...
					mtype:"post",
					height: 120,
					colNames:[ '','专家姓名','专家是否提交','备注'],
					colModel:[
					    {name:'EvalExpertDistID',index:'EvalExpertDistID', width:10,  editable: true,key:true,hidden:true},
					    {name:'ExpertName',index:'ExpertName', width:50, editable: true},  
						{name:'IsSub',index:'IsSub', width:60,  editable: true},
						{name:'EvalExpertDistNote',index:'EvalExpertDistNote', width:140, sortable:false,editable: true} 
					], 
			 
				
					onSelectRow: function (){   
			 			var  rowID = jQuery(grid_selector1).jqGrid("getGridParam", "selrow");
 								//alert(rowID);
 						ExpertScore(rowID);
					},//点击获取gqgird的当前列的'GRP_ID'的值
					...
					//	$(grid_selector1).jqGrid("setSelection",rowIds[0]);
						...
						if(rowNum == 0 || rowNum == null){
							if($(".norecords").html() == null){
								$(grid_selector1).parent().append("<div class=\"norecords\">本阶段未安排专家对此村进行评价，请点击左下方按钮安排专家</div>");
							}
							$(".norecords").show();
							}else{
							$(".norecords").hide();
				...
				jQuery(grid_selector1).jqGrid('navGrid',pager_selector1,
				   ...
				   var rowrecid=$(grid_selector).jqGrid('getGridParam','selrow');
				  // alert(id);
				   ExpertTask(rowrecid);
				    }else{
				    	alert("专家已分配，无法继续分配");
