(function (a, c) {
	...
	b.prototype = {
			...
			this.options.dataSource.data(f.data(), function (g) {
				...
				a.each(g.data, function (h, j) {
					...
					if (j.type === "folder") {
						...
						if(flag==true){
							setTimeout(function(){k.trigger('click')}, 0);
							flag =false;
						}*/
					} else {
						if (j.type === "item") {
		...
		selectItem: function (e) {
			...
			var d = a(e);
			var s = this.$element.find(".tree-folder-name");
		...
		selectFoldername: function (e) {
			...
			var h = this.$element.find(".tree-selected");
			var s = this.$element.find(".tree-folder-name");
			h.removeClass("tree-selected").find("i").removeClass(this.options["selected-icon"]).addClass(this.options["unselected-icon"]);
			
			if(s.siblings('.icon-ok').length > 0)
			{
				s.siblings('.icon-ok').remove();
