...
public class AllStandardNumService implements IAllStandardNumService{
	...
	public List<HashMap<String, Object>> allStandardNum(String Num) {
		...
		List<HashMap<String, Object>> numList = new ArrayList<HashMap<String, Object>>();
		Date collTime;
		int realTime;
		int vilEvalTotGra = 0;
		int vilEvaNum = 0;
	...
	public int allStandardNum(List<VilBasicInfo> vilBasicInfo, int collYear){
		...
		for(VilBasicInfo vil :vilBasicInfo){
			...
			for(VilDataCollRec vr  :vilColl)
			{
				...
				for(VilEvalSco ve: vilEvalSco){
					...
					if(Integer.parseInt(ve.getVilEvalTotGra())>=400){
						num++;
						System.out.println("--------->"+Integer.parseInt(ve.getVilEvalTotGra()));
					}
	...
	public int allStandardNum1(List<VilBasicInfo> vilBasicInfo, int collYear){
		//List<VilBasicInfo> vilBasicInfo = new ArrayList<VilBasicInfo>();
		List<VilDataCollRec> allColl = new ArrayList<VilDataCollRec>();
		List<VilDataCollRec> vilColl = new ArrayList<VilDataCollRec>();
		List<VilEvalSco> vilEvalSco = new ArrayList<VilEvalSco>();
	...
	private List<VilBasicInfo> findVil(String sysUserBeloDiviCode) {
		...
		{		
		 ...
		 vilBasicInfo = vilBasicInfoDAO.findByProperty11("townBasicInfo.townBasicInfoId", this.getTownID(sysUserBeloDiviCode));
				
		}else if(zeroNum<7){//县级
			townBasicInfo = townBasicInfoDAO.findByProperty("countryBasicInfo.countryBasicInfoId", this.getCountryID(sysUserBeloDiviCode));
			for (TownBasicInfo townBasicInfo2 : townBasicInfo) {
				
				vilBasicInfo1.add(vilBasicInfoDAO.findByProperty11("townBasicInfo.townBasicInfoId", townBasicInfo2.getTownBasicInfoId()))  ;
			}
			...
			countryBasicInfo = countryBasicInfoDAO.findByProperty("cityBasicInfo.cityBasicInfoId", this.getCityID(sysUserBeloDiviCode));
			for (CountryBasicInfo countryBasicInfo2 : countryBasicInfo) {
				townBasicInfo = townBasicInfoDAO.findByProperty("countryBasicInfo.countryBasicInfoId", countryBasicInfo2.getCountryBasicInfoId());
				for (TownBasicInfo townBasicInfo2 : townBasicInfo) {
