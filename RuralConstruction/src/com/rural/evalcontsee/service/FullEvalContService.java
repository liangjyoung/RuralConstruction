...
public class FullEvalContService implements IFullEvalContService{
	...
	public List<HashMap<String, Object>> queryByPage(List<Criterion> criteria,
		...
		try{
			String a=FullEvalContAction.getJSTime();
			a = URLDecoder.decode(a, "UTF-8");
		 //SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
		 SimpleDateFormat formatter = new SimpleDateFormat("yyyy");
		   //ParsePosition pos = new ParsePosition(0);
		
		 //Date sdate = new Date(a.trim().toString().substring(0, 4));
		 ...
		 int year = a1.get(Calendar.YEAR);  
		  ...
		  //vi = vilDataCollRecDAO.findVilDataCollRec(vilId, udate);
		...
		for(VilDataCollRec v:vi){
			 ...
			 list2 = this.evalContSubScoDAO.findByConditionAndPager(vilEvalScoId,sql, values, from,length);
				for(EvalContSubSco ev:list2){
					...
					if(Integer.parseInt(evalCont.getEvalContScore())==Integer.parseInt(ev.getAutoEvalSco())){
